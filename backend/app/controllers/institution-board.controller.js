...
exports.create = (req, res) => {
  ...
  Institution_board.destroy({
    where: { inst_id: inst_id }
  }).then(d => {

    req.body.institution_boards.forEach(institution_board =>  {

      if (!institution_board.inst_id) {
        res.status(400).send({
          message: "Content can not be empty!"
        });
        return;
  ...
  // Save City in the database
    Institution_board.create(institution_board)
      .then(data => {
...
exports.update = (req, res) => {
  ...
  Institution_board.update(req.body.institutionboard, {
    where: { id: id }
  })
    .then(num => {
      if (num == 1) {
...
exports.findAllPublished = (req, res) => {
  Institution_board.findAll({ where: { is_active: true } })
    .then(data => {
      res.send(data);
    })
    .catch(err => {
