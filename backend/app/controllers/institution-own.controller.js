...
const Institution_own = globalMastersDB.institution_own;
...
exports.create = (req, res) => {
  ...
  Institution_own.create(institution_type)
    .then(data => {
      res.send(data);
    })
    .catch(err => {
...
exports.update = (req, res) => {
  ...
  Institution_own.update(req.body, {
    where: { id: id }
  })
    .then(num => {
      if (num == 1) {
...
exports.findAllPublished = (req, res) => {
  Institution_own.findAll({ where: { is_active: true } })
    .then(data => {
      res.send(data);
    })
    .catch(err => {
